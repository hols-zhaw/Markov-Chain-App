...
st.markdown(
    "Define your states and transition probabilities, then run simulations and visualize results."
)
...
visual_type = st.radio(
    "Visualization type", ["Single trajectory", "Distribution"], horizontal=True
)
...
if st.button("Run Simulation"):
    if visual_type == "Single trajectory":
        ...
        traj_states = [states[i] for i in traj]
        st.write("Trajectory:")
        st.write(" → ".join(traj_states))
        st.line_chart(pd.Series([states[i] for i in traj]).astype("category").cat.codes)

...
for i, row in enumerate(states):
    ...
    with st.expander(f"Row: {row}", expanded=True):
        ...
        for j, col in enumerate(states):
            key = f"prob_{i}_{j}"
            if i == j:
                values.append(0.0)  # diagonal locked
            else:
                val = st.number_input(
                    f"P({row}→{col})",
                    min_value=0.0,
...
def render_pyvis_graph(matrix, states):
    ...
    for i, from_state in enumerate(states):
        for j, to_state in enumerate(states):
            prob = matrix[i, j]
            label = f"{prob:.2f}"
            net.add_edge(from_state, to_state, value=prob, label=label, arrows="to")
    ...
    os.remove(tmp_path)  # Optional: clean up


st.subheader("📉 Markov Chain Graph")
render_pyvis_graph(st.session_state.matrix.values, st.session_state.states)
...
visual_type = st.radio(
    "Visualization type",
    ["Single trajectory", "Distribution", "Mean & Std over time"],
    horizontal=True,
)
...
if st.button("Run Simulation"):
    if visual_type == "Single trajectory":
        ...
        st.session_state.mean_plot_data = simulate_many_chains(
            P, num_reps, num_steps, start_idx
        )
        st.session_state.simulation_result = None
        st.session_state.heatmap_data = None
...
# Result Rendering
if (
    visual_type == "Single trajectory"
    and st.session_state.simulation_result is not None
):
    ...
    ax.fill_between(
        range(len(mean)), mean - std, mean + std, alpha=0.2, label="±1σ", color="blue"
    )
    ax.fill_between(
        range(len(mean)), mean - ci, mean + ci, alpha=0.3, label="99% CI", color="gray"
    )
